...
try {
    $registerResult = Invoke-RestMethod -Uri "$BaseUrl/api/v1/auth/register" -Method POST -Body $registerBody -ContentType "application/json" -Headers $BypassHeader -ErrorAction Stop
...
$loginResult = Invoke-RestMethod -Uri "$BaseUrl/api/v1/auth/login" -Method POST -Body $loginBody -ContentType "application/json" -Headers $BypassHeader -ErrorAction Stop
...
$productResult = Invoke-RestMethod -Uri "$BaseUrl/api/v1/products" -Method POST -Body $productBody -ContentType "application/json" -Headers (@{ Authorization = "Bearer $Token" } + $BypassHeader) -ErrorAction SilentlyContinue
if ($productResult) {
...
try {
    Invoke-RestMethod -Uri "$BaseUrl/api/v1/inventory/$ProductId/add" -Method POST -Body $stockBody -ContentType "application/json" -Headers (@{ Authorization = "Bearer $Token" } + $BypassHeader) -ErrorAction Stop | Out-Null
    Write-Ok "库存添加成功"
} catch {
...
oha -z $Duration -c $c -m POST -d '{\"email\":\"$TestEmail\",\"password\":\"$TestPassword\"}' -H 'Content-Type: application/json' $OhaBypass "$BaseUrl/api/v1/auth/login"
"@
}

# 2.2 查询商品（GET + DB 读 + JWT 验证）
foreach ($c in $Concurrent) {
    Run-Bench "查询商品 - ${c}并发 x ${Duration}" "oha -z $Duration -c $c -H 'Authorization: Bearer $Token' $OhaBypass `"$BaseUrl/api/v1/products/$ProductId`""
}

# 2.3 查询库存（GET + DB 读）
foreach ($c in $Concurrent) {
    Run-Bench "查询库存 - ${c}并发 x ${Duration}" "oha -z $Duration -c $c -H 'Authorization: Bearer $Token' $OhaBypass `"$BaseUrl/api/v1/inventory/$ProductId`""
}

# 2.4 商品列表（GET + DB 分页查询）
foreach ($c in $Concurrent) {
    Run-Bench "商品列表 - ${c}并发 x ${Duration}" "oha -z $Duration -c $c -H 'Authorization: Bearer $Token' $OhaBypass `"$BaseUrl/api/v1/products?page=1&page_size=20`""
}

# 2.5 创建订单（POST + DB 写 + Saga + 库存扣减）
Run-Bench "创建订单 - 10并发 x ${Duration}（写操作降并发）" @"
oha -z $Duration -c 10 -m POST -d '{\"user_id\":$UserId,\"items\":[{\"product_id\":$ProductId,\"quantity\":1,\"unit_price\":99.99}]}' -H 'Content-Type: application/json' -H 'Authorization: Bearer $Token' $OhaBypass "$BaseUrl/api/v1/orders"
"@
